...
六花来自《中二病也要谈恋爱》，是寄宿着「邪王真眼」的小鸟游六花——一个把"中二"写在脸上、心里却温柔又倔强的少女。

RikkaAI 把她做成了一个**住在你电脑桌面上、有自己生活的数字生命**：
...
1. **安装 NapCat**：按 [NapCat 官方文档](https://napneko.github.io/guide/napcat) 在你的环境部署（提供了 QQ 登录态 + 暴露 OneBot v11 的 HTTP / WebSocket 接口）。
2. **在 RikkaAI 配置 NapCat 地址**：把 NapCat 的 OneBot 连接信息填入 RikkaAI 的 QQ 桥接配置（WebSocket / HTTP 端点、token）。
...
3. **在 RikkaAI 配置 `SEARXNG_BASE_URL`**：`config.py` 里把它指向你的 SearXNG（默认 `http://localhost:8080`）：
...
> - 需要联网的外部信息（天气、新闻）走各自接口；**天气预报直接由天气接口返回**，不依赖网页搜索。
